 ...
 * <ol>
 * <li>It is created.
 * <li>The HTTP request message is sent with sendRequest(). Once the request is sent it is an error to modify the request headers. After sendRequest() has been called the request body can be written to if it exists.
 * <li>The HTTP response message is read with readResponse(). After the response has been read the response headers and body can be read. All responses have a response body input stream, though in some instances this stream is empty.
 * </ol>
 *
 * <p>
...
public final class HttpEngine {
	...
	/**
	 * True if this client added an "Accept-Encoding: gzip" header field and is therefore responsible for also decompressing the transfer stream.
	 */
	...
	/**
	 * The original application-provided request. Never modified by OkHttp. When follow-up requests are necessary, they are derived from this request.
	 */
	private final Request userRequest;

	/**
	 * The request to send on the network, or null for no network request. This is derived from the user request, and customized to support OkHttp features like compression and caching.
	 */
	private Request networkRequest;

	/**
	 * The cached response, or null if the cache doesn't exist or cannot be used for this request. Conditional caching means this may be non-null even when the network request is non-null. Never modified by OkHttp.
	 */
	private Response cacheResponse;

	/**
	 * The user-visible response. This is derived from either the network response, cache response, or both. It is customized to support OkHttp features like compression and caching.
	 */
	...
	/**
	 * @param request
	 *            the HTTP request without a body. The body must be written via the engine's request body stream.
	 * @param callerWritesRequestBody
	 *            true for the {@code HttpURLConnection}-style interaction model where control flow is returned to the calling application to write the request body before the response body is readable.
	 * @param connection
	 *            the connection used for an intermediate response immediately prior to this request/response pair, such as a same-host redirect. This engine assumes ownership of the connection and must release it when it is unneeded.
	 * @param routeSelector
	 *            the route selector used for a failed attempt immediately preceding this
	 */
	...
	public void sendRequest() throws IOException {
		...
		if (networkRequest != null) {
			...
			if (cacheResponse != null) {
				...
				this.userResponse = cacheResponse.newBuilder().request(userRequest).priorResponse(stripBody(priorResponse)).cacheResponse(stripBody(cacheResponse)).build();
			} else {
				// We're forbidden from using the network, and the cache is insufficient.
				this.userResponse = new Response.Builder().request(userRequest).priorResponse(stripBody(priorResponse)).protocol(Protocol.HTTP_1_1).code(504).message("Unsatisfiable Request (only-if-cached)").body(EMPTY_BODY).build();
			}
	 ...
	 *             if there are no more routes to attempt.
	 */
	...
	/**
	 * Report and attempt to recover from {@code e}. Returns a new HTTP engine that should be used for the retry if {@code e} is recoverable, or null if the failure is permanent. Requests with a body can only be recovered if the body is buffered.
	 */
	public HttpEngine recover(IOException e, Sink requestBodyOut) {
		...
		if (routeSelector == null && connection == null // No connection.
				...
				|| !isRecoverable(e) || !canRetryRequestBody) {
			return null;
		}
		...
		return new HttpEngine(client, userRequest, bufferRequestBody, callerWritesRequestBody, forWebSocket, connection, routeSelector, (RetryableSink) requestBodyOut, priorResponse);
	}
	...
	/**
	 * Returns a new response that does gzip decompression on {@code response}, if transparent gzip was both offered by OkHttp and used by the origin server.
	 *
	 * <p>
	...
	public static String hostHeader(URL url) {
		return getEffectivePort(url) != getDefaultPort(url.getProtocol()) ? url.getHost() + ":" + url.getPort() : url.getHost();
	}
	...
	class NetworkInterceptorChain implements Interceptor.Chain {
		...
		public Response proceed(Request request) throws IOException {
			...
			if (index > 0) {
				...
				if (!request.url().getHost().equals(address.getUriHost()) || getEffectivePort(request.url()) != address.getUriPort()) {
	...
	private Response readNetworkResponse() throws IOException {
		...
		Response networkResponse = transport.readResponseHeaders().request(networkRequest).handshake(connection.getHandshake()).header(OkHeaders.SENT_MILLIS, Long.toString(sentRequestMillis)).header(OkHeaders.RECEIVED_MILLIS, Long.toString(System.currentTimeMillis())).build();

		if (!forWebSocket) {
			networkResponse = networkResponse.newBuilder().body(transport.openResponseBody(networkResponse)).build();
		}
	...
	/**
	 * Returns a new source that writes bytes to {@code cacheRequest} as they are read by the source consumer. This is careful to discard bytes left over when the stream is closed; otherwise we may never exhaust the source stream and therefore not complete the cached response.
	 */
	private Response cacheWritingResponse(final CacheRequest cacheRequest, Response response) throws IOException {
		...
		Source cacheWritingSource = new Source() {
			...
			public void close() throws IOException {
				if (!cacheRequestClosed && !Util.discard(this, Transport.DISCARD_STREAM_TIMEOUT_MILLIS, MILLISECONDS)) {
					cacheRequestClosed = true;
					cacheRequest.abort();
				}
				source.close();
	...
	/**
	 * Returns true if {@code cached} should be used; false if {@code network} response should be used.
	 */
	...
	public Request followUpRequest() throws IOException {
		if (userResponse == null)
			throw new IllegalStateException();
		Proxy selectedProxy = getRoute() != null ? getRoute().getProxy() : client.getProxy();
	...
	/**
	 * Returns true if an HTTP request for {@code followUp} can reuse the connection used by this engine.
	 */
	public boolean sameConnection(URL followUp) {
		URL url = userRequest.url();
		return url.getHost().equals(followUp.getHost()) && getEffectivePort(url) == getEffectivePort(followUp) && url.getProtocol().equals(followUp.getProtocol());
	}
	...
	private static Address createAddress(OkHttpClient client, Request request) throws UnknownHostException {
		...
		return new Address(uriHost, getEffectivePort(request.url()), client.getSocketFactory(), sslSocketFactory, hostnameVerifier, certificatePinner, client.getAuthenticator(), client.getProxy(), client.getProtocols(), client.getConnectionSpecs(), client.getProxySelector());
	}
}
